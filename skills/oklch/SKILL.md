...
- **Full script output:** Always include the **full** conversion table from the script (every row). Do not omit any rows. For rows where (1) the file is a documentation file and (2) the color is a usage example on that line, **annotate** the row (e.g. add a column or note: `documentation example`).
